...
tellraw @a ['',{text:'---------------------------------'}]
tellraw @a ['',{text:'  中心座標 : '},{translate:'[%s, %s, %s]',with:[{score:{name:'#mad',objective:'PosX'}},{score:{name:'#mad',objective:'PosY'}},{score:{name:'#mad',objective:'PosZ'}}],hover_event:{action:'show_text',value:{text:'スペクテイター時、\nクリックでテレポート'}},click_event:{action:'run_command',command:'/trigger TeleportToWorldCenter set 1'},color:'green'}]
