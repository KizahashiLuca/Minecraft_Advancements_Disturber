#####################################
## Minecraft Advancements Disturber
## MC-Version: Java Edit. 1.20.3
## Author    : @potagegatop
## Author    : @KizahashiLuca
## Date      : 06 Dec 2023
## Version   : β-2.5
## Licensed under CC BY-SA 4.0. 
#####################################

## プレイヤータグ削除
tag @a remove mad_host
tag @a remove mad_player
tag @a remove MAD_Spectator
tag @a remove MAD_PlayerToLeaveTeam
tag @a remove mad_team_a
tag @a remove mad_team_b
tag @a remove mad_team_c
tag @a remove mad_team_d
tag @a remove mad_team_no
tag @a remove MAD_Player1
tag @a remove MAD_Player2
tag @a remove MAD_Player3
tag @a remove MAD_Player4
tag @a remove MAD_Player5
tag @a remove MAD_Player6
tag @a remove MAD_Player7
tag @a remove MAD_Player8
tag @a remove MAD_Player9
tag @a remove MAD_Player10
tag @a remove MAD_Player11
tag @a remove MAD_Player12
tag @a remove MAD_Player13
tag @a remove MAD_Player14
tag @a remove MAD_Player15
tag @a remove MAD_Player16
tag @a remove MAD_Player17
tag @a remove MAD_Player18
tag @a remove MAD_Player19
tag @a remove MAD_Player20
tag @a remove MAD_GiveTimeRandomly

tag @a remove MAD_TransmitPlayer
tag @a remove MAD_EmergencyCallee
tag @a remove MAD_DetectEmergencyCall
tag @a remove MAD_Thief
tag @a remove MAD_DetectThief
tag @a remove MAD_DetectThiefTeam
tag @a remove MAD_SetThief
tag @a remove MAD_NotStealPlayer
tag @a remove MAD_StolenPlayer
tag @a remove MAD_ThiefTeam
tag @a remove MAD_DetectMazeMaker
tag @a remove MAD_MazePlayer
tag @a remove MAD_ArrowTeleportee
tag @a remove MAD_RespawnBannerSet
tag @a remove MAD_RespawnBannerSetTmp
tag @a remove MAD_DetectedBannerOwner
tag @a remove MAD_MobileRespawnBannerSet
tag @a remove MAD_MobileRespawnBannerSetTmp
tag @a remove MAD_DetectMobileBeaconTag
tag @a remove MAD_DetectRespawnPlayer
tag @a remove MAD_SetRespawnPlayer