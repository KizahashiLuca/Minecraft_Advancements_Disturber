...
scoreboard objectives add NumOfDeaths minecraft.custom:minecraft.deaths ['',{text:'デス数'}]
...
scoreboard objectives add TeleportToCarePackage trigger ['',{text:'トリガー - 支援物資へのテレポート'}]
scoreboard objectives add TeleportToWorldCenter trigger ['',{text:'トリガー - 原点へのテレポート'}]
...
#### 支援物資用 - どっか行け矢(仮)
scoreboard objectives add ArrowTeleportTick dummy ['',{text:'どっか行け矢(仮) - 経過ティック数'}]
scoreboard objectives add ArrowTeleportPosX dummy ['',{text:'どっか行け矢(仮) - テレポート先 - X座標'}]
scoreboard objectives add ArrowTeleportPosY dummy ['',{text:'どっか行け矢(仮) - テレポート先 - Y座標'}]
scoreboard objectives add ArrowTeleportPosZ dummy ['',{text:'どっか行け矢(仮) - テレポート先 - Z座標'}]
#### リスポーンバナー
