...
execute if predicate mad:gamerule/match_mode/individual/ run \
  data modify storage mad: rules.match_mode.type set value 'individual'
execute if predicate mad:gamerule/match_mode/team run \
  data modify storage mad: rules.match_mode.type set value 'team'
#### 生存者数スコアボード - ゲーム終了処理用(スコアボード名の置換)
execute if predicate mad:gamerule/match_mode/individual/ run \
  data modify storage mad: rules.match_mode.number_of_living set value 'NumberOfPlayers'
execute if predicate mad:gamerule/match_mode/team run \
  data modify storage mad: rules.match_mode.number_of_living set value 'NumberOfTeams'
#### ワールド範囲 - スポーン範囲用(ゲームルールコマンドの置換)
data modify storage mad: rules.spawn_radius set from storage mad: rules.world_border
...
#### ワールド範囲 - ワールドボーダー設定用
data modify storage mad: world_spawn.overworld.world_border set from storage mad: rules.world_border
data modify storage mad: world_spawn.the_nether.world_border set from storage mad: rules.world_border
data modify storage mad: world_spawn.the_end.world_border set from storage mad: rules.world_border
#### 死亡メッセージ - ゲームルールコマンドの置換
execute if predicate mad:gamerule/match_mode/individual/ run \
  data modify storage mad: rules.show_death_messages set value 'false'
execute if predicate mad:gamerule/match_mode/team run \
  data modify storage mad: rules.show_death_messages set from storage mad: rules.death_message_visibility
#### 当たり判定 - true/falseの文字列で格納されているため変換
execute if data storage mad: {rules:{collision_rule:'true'}} run \
  data modify storage mad: rules.collision_rule set value 'always'
execute if data storage mad: {rules:{collision_rule:'false'}} run \
  data modify storage mad: rules.collision_rule set value 'pushOtherTeams'
#### ネームタグ表示 - true/falseの文字列で格納されているため変換
execute if data storage mad: {rules:{nametag_visibility:'true'}} run \
  data modify storage mad: rules.nametag_visibility set value 'hideForOtherTeams'
execute if data storage mad: {rules:{nametag_visibility:'false'}} run \
  data modify storage mad: rules.nametag_visibility set value 'never'
#### 死亡メッセージ - true/falseの文字列で格納されているため変換
execute if data storage mad: {rules:{death_message_visibility:'true'}} run \
  data modify storage mad: rules.death_message_visibility set value 'hideForOtherTeams'
execute if data storage mad: {rules:{death_message_visibility:'false'}} run \
  data modify storage mad: rules.death_message_visibility set value 'never'
