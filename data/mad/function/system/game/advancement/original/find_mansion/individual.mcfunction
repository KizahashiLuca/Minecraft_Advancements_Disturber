...
tellraw @s ['',{translate:'chat.type.advancement.task',with:[{selector:'@s'},{translate:'[%s]',color:'green',with:[{translate:'中は思ったよりきれいだな',hover_event:{action:'show_text',value:[{translate:'%s\n%s',color:'green',with:[{translate:'中は思ったよりきれいだな'},{translate:'森の洋館に侵入する'}]}]}}]}]}]
