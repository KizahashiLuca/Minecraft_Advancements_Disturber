...
## スコアボード設定
scoreboard players set @s Advancements_Adventure_trim_with_any_armor_pattern 1
...
tellraw @s ['',{translate:'chat.type.advancement.task',with:[{selector:'@s'},{translate:'[%s]',color:'green',with:[{translate:'advancements.adventure.trim_with_any_armor_pattern.title',hover_event:{action:'show_text',value:[{translate:'%s\n%s',color:'green',with:[{translate:'advancements.adventure.trim_with_any_armor_pattern.title'},{translate:'advancements.adventure.trim_with_any_armor_pattern.description'}]}]}}]}]}]
