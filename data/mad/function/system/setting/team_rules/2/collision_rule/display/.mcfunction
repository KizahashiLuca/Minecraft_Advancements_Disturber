...
execute if predicate mad:gamerule/team_rules/2/collision_rule/false run execute positioned ~ ~1.00 ~ run function mad:system/setting/team_rules/2/collision_rule/display/true {color1:'gray',color2:'dark_green',underlined:'true',text:'クリックして変更',background:'-1'}
#### 当たり判定なし
execute if predicate mad:gamerule/team_rules/2/collision_rule/true run execute positioned ~ ~0.50 ~ run function mad:system/setting/team_rules/2/collision_rule/display/false {color1:'gray',color2:'dark_red',underlined:'true',text:'クリックして変更',background:'-1'}
