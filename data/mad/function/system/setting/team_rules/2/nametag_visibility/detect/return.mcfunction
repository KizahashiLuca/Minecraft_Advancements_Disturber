...
## エンティティ削除
#### テキストディスプレイ
kill @e[tag=MAD_TextDisplay_Setting_nametagVisibility]
#### インタラクション
kill @e[tag=MAD_Interaction,tag=!MAD_Interaction_Setting_ChangeSpectator]

## ネームタグ表示変更メッセージ表示
execute if predicate mad:gamerule/team_rules/2/nametag_visibility/true run function mad:message/setting/team_rules/2/nametag_visibility/ {text:'あり',color:'dark_green'}
execute if predicate mad:gamerule/team_rules/2/nametag_visibility/false run function mad:message/setting/team_rules/2/nametag_visibility/ {text:'なし',color:'dark_red'}
