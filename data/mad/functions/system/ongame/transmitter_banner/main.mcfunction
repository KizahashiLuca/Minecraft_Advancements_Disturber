...
## Send messages
execute if entity @e[type=minecraft:armor_stand,tag=RespawnBanner,tag=DetectTransmitter] run tellraw @s ["",{"text":"[発 信 機] 仲間のリスポーンバナーが ","color":"green"},{"translate":"[%s, %s, %s]","with":[{"score":{"name":"@s","objective":"TransmitterPosX"}},{"score":{"name":"@s","objective":"TransmitterPosY"}},{"score":{"name":"@s","objective":"TransmitterPosZ"}}],"color":"green"},{"text":" にあります。","color":"green"}]
