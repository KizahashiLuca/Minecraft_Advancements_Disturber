...
execute as @a[tag=RespawnTeam,scores={Phase=22,Death=1..3}] run scoreboard players add @p[tag=RespawnTeam,tag=Leader] NumDead 1
scoreboard players set #mad NumOfRespawn 1
scoreboard players set @a[tag=RespawnTeam,scores={Phase=22,Death=1..3}] NumOfRespawn 0
function mad:system/ongame_team/respawn_beacon/numbering
tag @a[tag=RespawnTeam] remove Numbered

## Setblock
execute at @e[type=minecraft:area_effect_cloud,nbt={Tags:["RespawnBeacon"]}] run fill ~-1 ~ ~-1 ~1 ~2 ~1 minecraft:air
execute at @e[type=minecraft:area_effect_cloud,nbt={Tags:["RespawnBeacon"]}] run function mad:system/ongame_team/respawn_beacon/set_beacon
...
## Send message
scoreboard players set @s TargetOfRespawn 0
