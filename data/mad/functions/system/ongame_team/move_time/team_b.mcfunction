...
execute as @s[team=!TeamB] run tellraw @a[team=TeamB] ["",{"text":"[時間移動]  ","color":"green"},{"selector":"@s","bold":true},{"text":" から ","color":"green"},{"score":{"name":"@p[tag=Leader,tag=CalculateTime]","objective":"SecondPerSurvive"},"color":"green","bold":true},{"text":" 秒奪いました。","color":"green"}]
