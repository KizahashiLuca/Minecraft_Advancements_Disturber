...
## Scoreboard
scoreboard players operation @p[team=TeamA,tag=Leader] Second += @p[tag=Leader,tag=CalculateTime] SecondPerSurvive
scoreboard players operation @p[team=TeamA,tag=Leader] TimeLimit += @p[tag=Leader,tag=CalculateTime] SecondPerSurvive
scoreboard players operation @p[team=TeamA,tag=Leader] GetTimeLimit += @p[tag=Leader,tag=CalculateTime] SecondPerSurvive
...
execute as @s[team=TeamE] run tellraw @a[team=TeamE] ["",{"text":"[時間移動]  ","color":"green"},{"text":"赤チーム","color":"red","bold":true},{"text":" に ","color":"green"},{"score":{"name":"@p[tag=Leader,tag=CalculateTime]","objective":"SecondPerSurvive"},"color":"green","bold":true},{"text":" 秒奪われました。","color":"green"}]
...
## Revoke advancements
advancement revoke @a only mad:system/ongame_team/be_hit_by_team_a
