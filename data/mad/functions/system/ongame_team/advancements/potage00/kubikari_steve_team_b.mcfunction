...
advancement grant @a[team=TeamB] only mad:potage00/kubikari_steve
tellraw @a[team=TeamB] ["",{"translate":"chat.type.advancement.task","with":[{"text":"青チーム","color":"blue","bold":true},{"translate":"[%s]","color":"green","with":[{"text":"首狩りスティーブ","hoverEvent":{"action":"show_text","value":[{"translate":"%s\n%s","color":"green","with":[{"text":"首狩りスティーブ"},{"text":"スティーブの頭を手に入れろ！"}]}]}}]}]}]
