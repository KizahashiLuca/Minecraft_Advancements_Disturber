...
## Set scoreboard
scoreboard players set @a[team=TeamA] AD_Story_lava_bucket 1
...
tellraw @a[team=TeamA] ["",{"translate":"chat.type.advancement.task","with":[{"text":"紫チーム","color":"dark_purple","bold":true},{"translate":"[%s]","color":"green","with":[{"translate":"advancements.story.lava_bucket.title","hoverEvent":{"action":"show_text","value":[{"translate":"%s\n%s","color":"green","with":[{"translate":"advancements.story.lava_bucket.title"},{"translate":"advancements.story.lava_bucket.description"}]}]}}]}]}]
