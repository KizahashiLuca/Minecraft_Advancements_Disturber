...
## Set scoreboard
scoreboard players set @a[team=TeamA] AD_Adventure_honey_block_slide 1
...
tellraw @a[team=TeamA] ["",{"translate":"chat.type.advancement.task","with":[{"text":"紫チーム","color":"dark_purple","bold":true},{"translate":"[%s]","color":"green","with":[{"translate":"advancements.adventure.honey_block_slide.title","hoverEvent":{"action":"show_text","value":[{"translate":"%s\n%s","color":"green","with":[{"translate":"advancements.adventure.honey_block_slide.title"},{"translate":"advancements.adventure.honey_block_slide.description"}]}]}}]}]}]
