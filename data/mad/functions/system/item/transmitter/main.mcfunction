...
tag @p[predicate=mad:system/item/transmitter/detect_transmitter,sort=nearest,limit=1] add MAD_TransmitPlayer
...
## Send messages
execute if entity @e[predicate=mad:system/item/transmitter/transmit_player] run tellraw @s ["",{"translate":"[発 信 機]  %s が ","with":[{"selector":"@p[predicate=mad:system/item/transmitter/transmit_player]"}],"color":"green","bold":false,"italic":false},{"translate":"[%s, %s, %s]","with":[{"score":{"name":"@s","objective":"TransmitterPosX"}},{"score":{"name":"@s","objective":"TransmitterPosY"}},{"score":{"name":"@s","objective":"TransmitterPosZ"}}],"color":"green","bold":false,"italic":false},{"text":" にいます。","color":"green","bold":false,"italic":false}]
