...
execute as @s[predicate=mad:system/item/maze_maker/angle/east,scores={MazePosX=..7}] at @s run fill ~ ~ ~-1 ~5 ~3 ~1 minecraft:air replace #mad:wall_block
execute as @s[predicate=mad:system/item/maze_maker/angle/south,scores={MazePosZ=..7}] at @s run fill ~-1 ~ ~ ~1 ~3 ~5 minecraft:air replace #mad:wall_block