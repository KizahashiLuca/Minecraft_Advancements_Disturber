...
execute if entity @p[predicate=mad:system/item/notice_of_thief/stolen_player] as @s[predicate=mad:system/item/notice_of_thief/result/failure] run tellraw @a[predicate=mad:system/item/notice_of_thief/detect_not_thief] ["",{"translate":"[怪盗予告]  %s の手持ちアイテムが無いため、アイテムは盗まれませんでした。","with":[{"selector":"@p[predicate=mad:system/item/notice_of_thief/stolen_player]"}],"color":"green","bold":false,"italic":false}]
execute unless entity @p[predicate=mad:system/item/notice_of_thief/stolen_player] run tellraw @a[predicate=mad:system/item/notice_of_thief/detect_not_thief] ["",{"text":"[怪盗予告] 盗み先を指定できず、アイテムは盗まれませんでした。","color":"green","bold":false,"italic":false}]