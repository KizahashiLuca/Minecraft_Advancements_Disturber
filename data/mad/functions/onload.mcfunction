...
tellraw @a[predicate=mad:gamerules/num_of_participants/one] ["",{"translate":"     >> %s","with":[{"text":"ソロ戦","color":"green","bold":true,"underlined":true,"hoverEvent":{"action":"show_text","value":"クリックして個人戦開始"},"clickEvent":{"action":"run_command","value":"/function mad:system/begin/start_game/solo/main"}}],"color":"white","bold":false,"italic":false}]
